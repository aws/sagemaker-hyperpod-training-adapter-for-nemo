...
class TestSageMakerFullCheckpointIO:
    ...
    def mock_get_rank(sefl, monkeypatch):
        mock_rank = MagicMock()
        monkeypatch.setattr("sagemaker_nemo_adaptor.utils.callbacks.full_ckpt_io.dist.get_rank", mock_rank)
        return mock_rank

    @patch("sagemaker_nemo_adaptor.utils.callbacks.full_ckpt_io.dist.get_rank")
